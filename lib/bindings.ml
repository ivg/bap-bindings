...
module Make( Internal : sig
    ...
    val declare :
      ...
      ?errorable : 'a Or_error.t C.typ ->
      ?subtypeof:_ ctype ->
      ?parent:_ ctype ->
      string -> 'a ctype

    val define : 'a ctype -> api list -> 'a ctype

    val enum : ?doc:string -> (module Enumerable with type t = 'a) ->
      string -> 'a ctype
    val def : ?doc:string -> string -> ('a -> 'b) C.fn -> ('a -> 'b) -> api

    (* val (!!) : ('a,'b) ctype -> 'a C.typ *)
    (* val (!?) : 'a nullable  -> 'a option C.typ *)

    val (@~>) : 'a ctype -> 'b C.fn -> ('a -> 'b) C.fn
    val partial : 'a ctype -> 'a option C.fn
    val returns : 'a ctype -> 'a C.fn
    val mayfail : 'a ctype -> 'a Or_error.t C.fn
    val typename : _ ctype -> string
    val global : ?doc:string -> string -> ('a -> 'b) C.fn -> ('a -> 'b) -> unit
end) =
struct
  ...
  let () =
    global "version" C.(void @-> returns cstring) version;
    global "_standalone_init" C.(int @-> ptr string @-> returning int)
      standalone_init
  ...
  let iterator seq elt =
    let t = declare ~parent:seq "iterator" in
    define t [
  ...
  let container
      ...
      ?instanceof elt : t ctype =
    let seq = declare ?i ~parent:elt "seq" in
    let apply hf seq f data = hf seq ~f:(fun elt -> f elt data) in
    let fn name rval = declare ~parent:seq name
        ~ctype:C.(fn (elt @~> ptr void @-> returning rval)) in
    let visitor = fn "visitor" C.void in
    let predicate = fn "predicate" C.bool in
    define seq [
      def "iter"
        C.(seq @~> visitor @~> ptr void @-> returning void)
        (apply Seq.iter);
      def "find"
        C.(seq @~> predicate @~> ptr void @-> partial elt.t)
        (apply Seq.find);
      def "exists"
  ...
  let seq (type t) (module T : T with type t = t) elt =
    let module Seq0 = struct
      type elt = t
      type t = elt seq
      include (Seq : Container.S1 with type 'a t := 'a seq)
    end in
    let seq = container ~instanceof:seq_base (module Seq0) elt in
    let iter = iterator seq elt in
    let mapper_c = fn C.(elt @~> ptr void @-> returns elt) in
    let mapper = declare ~parent:seq ~ctype:mapper_c "mapper" in
    let seq = define seq [
        def "map" C.(seq @~> mapper @~> ptr void @-> returns seq)
